...
def too_bright(detections: List[dict], bright_pixels: int = 70, threshold: int = 70) -> Tuple[List[dict], List[dict]]:
    """
    Analysis of the detection slice by checking the brightness of the pixels

    :param detections: list of detections
    :param bright_pixels: maximum number of bright pixels on the slice (wycinek)
    :param threshold: value of pixels for the bright_pixel (i.e bright pixel is when pixel have value >70)
    ...
    for image in detections:
        ...
        sum_value = int(np.sum(count_bright_pixels))
        get_and_set(image, BRIGHT_PIXELS, sum_value)
